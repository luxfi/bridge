...
const StarknetWalletWithdrawStep: FC<Props> = ({ depositAddress, amount }) => {
  ...
  const sourceCurrency = source_network?.assets.find(
    (c) => c.asset?.toLowerCase() === swap?.source_asset?.toLowerCase()
  );
  ...
  const handleTransfer = useCallback(async () => {
    ...
    try {
      ...
      if (!source_network?.metadata?.WatchdogContractAddress) {
        throw Error(
          "WatchdogContractAddress is not defined on network metadata"
        );
      }
      if (!amount) {
      ...
      const call = erc20Contract.populate("transfer", [
        depositAddress,
        parseInputAmountToUint256(amount.toString(), sourceCurrency?.decimals),
      ]);

      const watch = watchDogContract.populate("watch", [swap.sequence_number]);

      try {
        const { transaction_hash: transferTxHash } =
          (await wallet?.metadata?.starknetAccount?.account?.execute([
            call,
            watch,
          ])) || {};
        if (transferTxHash) {
        ...
        <div className="space-y-4">
          {isWrongNetwork && (
            <WarningMessage messageType="warning">
              <span className="flex">
                {source_network_internal_name ===
                KnownInternalNames.Networks.StarkNetMainnet ? (
                  <span>
                    Please switch to Starknet Mainnet with your wallet and click
                    Connect again
                  </span>
                ) : (
                  <span>
                    Please switch to Starknet Goerli with your wallet and click
                    Connect again
                  </span>
                )}
              </span>
