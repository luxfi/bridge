...
// the BridgeVM JSON-RPC interface (`${nodeUrl}/v1/chain/B/rpc`) and, when
// configured, the ThresholdVM interface (`${nodeUrl}/v1/chain/T/rpc`) for
...
// BridgeVM RPC at `${nodeUrl}/v1/chain/B/rpc`), not on ThresholdVM. They
...
type Client struct {
	...
	// (e.g. https://node.lux.network/v1/chain/B/rpc). Required.
	BridgeRPCURL string
...
func New(nodeURL string, timeout time.Duration) *Client {
	...
	return &Client{
		BridgeRPCURL:    nodeURL + "/v1/chain/B/rpc",
		ThresholdRPCURL: nodeURL + "/v1/chain/T/rpc",
		Timeout:         timeout,
	}
}
