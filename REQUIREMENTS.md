...
- The **frontend** is packaged as a publishable white-label SDK `@luxfi/bridge` (`pkg/bridge/`). The bridge UI is **inlined** under `pkg/bridge/src/app/` — there is no longer a workspace `@luxbridge/app-v3` dependency. Each tenant (Lux, Zoo, Hanzo, partner) consumes the SDK from a thin host shell plus its own `@<org>/brand` package.
...
5. Support partner as a third tenant operating its own backend (centralized MPC) — proves the SDK is genuinely backend-agnostic.
...
- Optimizing for partner's standalone-backend use case before Lux's hosted case works.
...
**Outcome:** `@luxfi/bridge` is published to the public npm registry and partner (or any third party) can adopt it with their own backend.
...
| 3.6 | Document standalone-backend mode for partner: how to point `apiHost` at a non-Lux endpoint, what server API contract `app/server/` exposes, what's pluggable. | New doc under `docs/content/docs/`. |
| 3.7 | Confirm partner can render their own securities (AAPL, etc.) via the existing tokens/assets registry (`@luxbridge/settings`) without forking. If not, add an injection point. | May surface a `tokens?` field on `BridgeConfig`. |
...
- A new standalone-backend doc clearly describes the partner path.
...
| R4 | partner's securities (AAPL, etc.) may require asset-registry injection points the current `@luxbridge/settings` does not expose. | Could force a SDK breaking change. | Investigate during Phase 3.7. |
...
1. Is Hanzo a tenant in this round, or only Lux + Zoo + (later) partner?
