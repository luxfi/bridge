...
| 6 | Non-EVM (Solana, BTC, TON, XRP) | 🟡 partial — **Solana** + **TON Testnet** + **XRP Testnet** shipped bidirectional 2026-06-04..2026-06-05 using `mpcd-single` (HKDF per-wallet ed25519, renamed + hardened from fake-mpcd on 2026-06-05) behind the family-routed `mpc-router`; production threshold ed25519 deferred to a separate cluster-FROST epic. **BTC** source-only enabled (deposit + balance display); withdrawal still gated on mpcd Schnorr + BTC tx assembler. `internal/txassembler/{ton,xrp}.go`, `internal/ton/{messaging,provider}.go`, `internal/xrp/{payment,account_id,provider}.go`, `cmd/bridge/refund_driver.go::executeRefund{TON,XRP}`, and the SOL counterparts all shipped — when cluster-FROST mpcd lands, the swap is just a router-route flip. |

### 13.7 Known gaps

| # | Gap | Impact |
|---|---|---|
| G1 | Non-EVM status (updated 2026-06-05): (a) ~~`broadcast/client.go` returns `ErrFamilyNotImplemented` for every non-EVM chain family~~ **fixed** — Solana + TON + XRP broadcast paths shipped; BTC broadcast still pending; (b) ~~signature parsing assumes ECDSA `R∥S∥V`~~ **fixed** — signing driver dispatches by destination family, ed25519 path produces raw 64-byte sigs; (c) ~~mpcd keygen returns `"no sol address"`~~ **worked around** — `mpc-router` dispatches ed25519 walletIDs to `mpcd-single` (HKDF per-wallet derivation); cluster-FROST mpcd parked as a separate epic; (d) ~~BTC mainnet-format address~~ **fixed 2026-05-28**. | Production SOL/TON/XRP using mpcd-single single-signer custody behind mpc-router; threshold custody is the cluster-FROST epic, tracked separately. BTC withdrawal gated on mpcd Schnorr + BTC tx assembler. |
