...
| Date | 2026-06-04 |
| Status | Draft for review (R7 — non-EVM corridor sweep. 2026-06-04 updates: Solana ↔ EVM bidirectional shipped (Sol → EVM source via Phantom auto-deposit, EVM → Sol destination via pure MPC SystemProgram.transfer, Sol → Lux testnet refund); BTC source-only enabled with balance display + sats-connect wallet hookup; TON Testnet ↔ Sepolia bidirectional shipped (V4R2 contract deploy + transfer, ed25519 cell-hash sign, BoC broadcast via toncenter, TonConnect deposit + refund sweep); `BRIDGE_ENV=local` sandbox tier added with Lux Local (31337) + Zoo Local (200203) for hermetic dev; HTTPS local access via nip.io reverse proxy (works around the wallet-popup origin requirement — Phantom/Tonkeeper silently hang on http+public-IP). MPC router at `/tmp/mpc-router` now dispatches by `wallet_id` family between fake-mpcd (ed25519 for Sol/TON) and real mpcd (ECDSA for EVM/BTC). §13.1 / §13.5 / §13.6 / §13.7 updated. Sections from R6 still hold: layered public/private pool, observability, LP-333, auto-expiry, KMS-ready secrets, Lux RPC proxy, E2E regression net.) |
...
| BTC corridor (source-only) | ✅ source enabled 2026-05-28, balance display 2026-06-03 | Deposit address derived in-bridge via `internal/mchain/btc_address.go` base58check re-encoder (mpcd still returns mainnet-format `1…`/`3…` P2PKH; bridge swaps version byte 0x00 → 0x6f to produce testnet `m…`/`n…`). Blockstream testnet API recognizes the re-encoded address. SPA balance display via `mempool.space/api/address/<addr>` through the family-aware `useWalletForBTC` hook; sats-connect wallet hookup added 2026-06-03. Backend addressMatchesType validates both bech32 (`bc1`/`tb1` + bech32 charset) and base58 (`1`/`3`/`m`/`n`/`2` + base58 charset) prefixes. **Withdrawal still blocked** — broadcast needs Schnorr signing in mpcd + a BTC tx assembler. |
...
| `BITCOIN_TESTNET` | bitcoin | BTC | — | 🟡 source-only (deposit + balance display enabled). Bridge re-encodes mpcd's mainnet-format P2PKH to testnet (`m…`/`n…`) via base58check version-byte swap. SPA balance via mempool.space. Withdrawal still blocked on mpcd Schnorr + BTC tx assembler. |
| `SOLANA_DEVNET` | solana | SOL | — | ✅ bidirectional (shipped 2026-06-04). Sol → EVM source via Phantom auto-deposit. EVM → Sol destination via pure MPC SystemProgram.transfer (ed25519 cluster, no Anchor program). Sol → Lux testnet refund leg verified. Family routing in `mpc-router` is required (`solana_*`/`sol_*` → eddsa cluster). |
| `TON_TESTNET` | ton | TON | — | ✅ bidirectional (shipped 2026-06-04). Sepolia ↔ TON Testnet end-to-end. V4R2 contract deploy + transfer, ed25519 cell-hash sign, BoC broadcast via toncenter. TonConnect/Tonkeeper wallet adapter. Family routing in `mpc-router` is required (`ton_*`/`-ton-*` → eddsa cluster). First successful release tx: `swap_9644f58db8883efd`. |

End-to-end signed swaps verified for: Sepolia ↔ LUX, Sepolia ↔ Solana Devnet, Sepolia ↔ TON Testnet (all both senses). The 5 EVM chains added 2026-05-28 all return correct CoinGecko-backed quotes and create swaps with valid MPC deposit addresses. BTC remains source-only pending mpcd Schnorr.
...
Reconciliation with `app/bridge/TESTNET-E2E.md` §2 is now complete for the EVM + Solana + TON corridors; BTC withdrawal is the lone remaining gap.
...
| 6 | Non-EVM (Solana, BTC, TON) | 🟡 partial — **Solana** + **TON Testnet** shipped bidirectional 2026-06-04 using fake-mpcd (ed25519) behind the family-routed `mpc-router`; production ed25519 still gated on mpcd FROST (task #112). **BTC** source-only enabled (deposit + balance display); withdrawal still gated on mpcd Schnorr + BTC tx assembler. `internal/txassembler/ton.go`, `internal/ton/{messaging,provider}.go`, `cmd/bridge/refund_driver.go::executeRefundTON`, and the SOL counterparts all shipped — when real mpcd FROST lands the swap is just a router-route flip. |

### 13.7 Known gaps

| # | Gap | Impact |
|---|---|---|
| G1 | Non-EVM status (updated 2026-06-04): (a) ~~`broadcast/client.go` returns `ErrFamilyNotImplemented` for every non-EVM chain family~~ **fixed** — Solana + TON broadcast paths shipped (`broadcastSolana` + `broadcastTON`); BTC broadcast still pending; (b) ~~signature parsing assumes ECDSA `R∥S∥V`~~ **fixed** — signing driver dispatches by destination family, ed25519 path produces raw 64-byte sigs; (c) ~~mpcd keygen returns `"no sol address"`~~ **worked around** — `mpc-router` dispatches ed25519 walletIDs to fake-mpcd; real mpcd FROST is still task #112 and is the prerequisite for production rollout of SOL/TON; (d) ~~BTC mainnet-format address~~ **fixed 2026-05-28**. | Production SOL/TON gated on real mpcd FROST (#112). BTC withdrawal gated on mpcd Schnorr + BTC tx assembler. |
...
| G7 | `mpc-router` is a dev-environment process at `/tmp/mpc-router.go` outside the main repo. | Required for any deploy that supports SOL or TON. Needs to be productionized (move into `cmd/mpc-router/`, ship in container image, add health endpoint) before mainnet enables SOL/TON. Today it works because the smoke environment runs it manually. |
