...
| Date | 2026-06-04 |
| Status | Draft for review (R8 — BTC bidirectional + cross-family BTC corridors + deposit-pool baseline fixes. 2026-06-08 updates: **BTC destination shipped** (`internal/btc/` package + `txassembler.PreSignBTC`/`FinalizeBTC` + `signing_driver` AddressTypeBTC case + `broadcast.broadcastBTC` via mempool.space + `--btc-rpc-{mainnet,testnet}-url` flags) — closes G1's "BTC withdrawal gated on mpcd Schnorr". Discovery: P2WPKH/P2PKH outputs need *only* regular secp256k1 ECDSA + BIP-143-style legacy sighash; no Schnorr/FROST work needed. P2TR (Taproot) outputs still deferred behind cluster-FROST. **BTC source refund** (`txassembler.PreSignBTCRefund` + `executeRefundBTC`) closes the BTC corridor symmetry — BTC→X swaps that fail post-deposit now sweep the user's testnet BTC back instead of stranding it. **XRP + TON source baseline fixes**: mpcd's XRPL / TON keygens reuse the long-lived release wallet's address for per-swap deposit wallets, so `depositcheck` auto-confirmed swaps the user never funded and the refund driver over-swept operator liquidity (96 XRP over-refund observed live during smoke). Added per-swap balance snapshot at create + delta-gated deposit check + capped refund (both XRP and TON). XRP regression-guarded by an explicit test that the new TON field doesn't leak into XRP logic. **Five bidirectional non-EVM corridors live-validated** end-to-end against BTC source/destination: ETH Sepolia↔BTC testnet3, BTC↔Lux testnet, BTC↔Sol Devnet, BTC↔XRP Testnet, BTC↔TON Testnet. §13.1 / §13.5 / §13.6 / §13.7 updated. Sections from R7 still hold: Solana + TON bidirectional, BRIDGE_ENV=local sandbox tier, HTTPS local access via nip.io proxy, MPC family router. Sections from R6 still hold: layered public/private pool, observability, LP-333, auto-expiry, KMS-ready secrets, Lux RPC proxy, E2E regression net.) |
...
| `SOLANA_DEVNET` | solana | SOL | — | ✅ bidirectional (shipped 2026-06-04). Sol → EVM source via Phantom auto-deposit. EVM → Sol destination via pure MPC SystemProgram.transfer (ed25519 cluster, no Anchor program). Sol → Lux testnet refund leg verified. Family routing in `mpc-router` is required (`solana_*`/`sol_*` → eddsa cluster). **2026-06-08:** Sol↔BTC bidirectional live-validated. |
...
| # | Gap | Impact |
|---|---|---|
| G1 | Non-EVM status (updated 2026-06-08): (a) ~~`broadcast/client.go` returns `ErrFamilyNotImplemented` for every non-EVM chain family~~ **fixed** — Solana + TON + XRP + **BTC** broadcast paths shipped; only DOT still pending; (b) ~~signature parsing assumes ECDSA `R∥S∥V`~~ **fixed** — signing driver dispatches by destination family, ed25519 path produces raw 64-byte sigs; (c) ~~mpcd keygen returns `"no sol address"`~~ **worked around** — `mpc-router` dispatches ed25519 walletIDs to `mpcd-single` (HKDF per-wallet derivation); cluster-FROST mpcd parked as a separate epic; (d) ~~BTC mainnet-format address~~ **fixed 2026-05-28**; (e) ~~BTC withdrawal blocked on mpcd Schnorr + BTC tx assembler~~ **fixed 2026-06-08** — turns out P2WPKH/P2PKH outputs need only regular ECDSA (no Schnorr); shipped `internal/btc/` package + assembler + ECDSA sign path + mempool.space broadcast (commit `f7fb55ff`). P2TR (Taproot) outputs still need Schnorr, deferred behind cluster-FROST. (f) ~~BTC source refund missing~~ **fixed 2026-06-08** — `executeRefundBTC` shipped (commit `403632a4`). (g) **NEW (closed 2026-06-08): XRP + TON deposit wallet shares its address with the long-lived release wallet** — mpcd's XRPL ed25519 / TON V4R2 keygens both reuse the release-wallet address for "per-swap deposit" minting, so `depositcheck` was auto-confirming swaps the user never funded and the refund driver was sweeping the operator's standing release-wallet liquidity to the user (96 XRP over-refund observed live in smoke testing). Fixed via per-swap balance snapshot at create + delta-gated deposit check + refund-amount cap (commits `c4b21b38` for XRP, `a489f5df` for TON, with explicit regression-guard test for XRP). | All non-EVM bidirectional corridors live as of 2026-06-08. SOL/TON/XRP using mpcd-single single-signer custody behind mpc-router; BTC using real mpcd (ECDSA); threshold custody for the ed25519 chains is the cluster-FROST epic, tracked separately. |
...
| **RBF / stale-tx rebuild for BTC release** in `broadcast_driver` (Sol has `handleStaleBlockhash`, TON has `handleStaleTONSeqno`, XRP has `handleStaleXRPSequence` — BTC has nothing) | Never hit live in this session; testnet fee market was quiet enough that release txs always confirmed on first broadcast. | A BTC release tx stuck in mempool during a real-network fee spike would re-broadcast the same tx forever instead of bumping the fee. Mainnet-only risk. |
...
| **BTC operator runbook + `bridge_btc_*` Prometheus metrics** | Production polish. XRP got a dedicated runbook section in #150 and mainnet metrics in #151. BTC didn't get the same treatment yet. | When ops is actively monitoring BTC release-wallet health, broadcast success rate, mempool.space latency. Useful for alerting on stuck releases or UTXO fragmentation before they cascade. |
