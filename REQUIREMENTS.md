...
- The **frontend** is being repackaged as a publishable white-label SDK `@luxfi/bridge` (`pkg/bridge/`). Each tenant (Lux, Zoo, Hanzo, redacted) consumes the SDK from a minimal `main.tsx` plus its own `@<org>/brand` package.
...
| 3.5 | Document standalone-backend mode for redacted: how to point `apiHost` at a non-Lux endpoint, what server API contract `app/server/` exposes, what's pluggable. | New doc: `docs/STANDALONE-BACKEND.md`. |
| 3.6 | Confirm redacted can render their own securities (AAPL, etc.) via the existing tokens/assets registry (`@luxbridge/settings`) without forking. If not, add an injection point. | May surface a `tokens?` field on `BridgeConfig`. |
...
- A new `docs/STANDALONE-BACKEND.md` clearly describes the redacted path.
...
| R5 | redacted's securities (AAPL, etc.) may require asset-registry injection points the current `@luxbridge/settings` does not expose. | Could force a SDK breaking change. | Investigate during Phase 3.6. |
...
2. Is Hanzo a tenant in this round, or only Lux + Zoo + (later) redacted? Sponsor transcript mentions Hanzo tooling but no tenant repo plan.
