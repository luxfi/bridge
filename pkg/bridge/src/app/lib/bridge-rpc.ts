...
// JSON-RPC interface at `${nodeUrl}/ext/bc/B/rpc`. MPC signing rounds run
// through the matching ThresholdVM interface (`/ext/bc/T/rpc`), handled by
...
export interface BridgeRPCClientOptions {
  /** B-Chain (BridgeVM) JSON-RPC URL, e.g. `https://node.lux.network/ext/bc/B/rpc`. */
  bridgeRpcUrl: string
...
export function createBridgeRPCClient(opts: {
  ...
  const init: BridgeRPCClientOptions = {
    bridgeRpcUrl: opts.bridgeRpcUrl ?? `${nodeUrl}/ext/bc/B/rpc`,
    thresholdRpcUrl: opts.thresholdRpcUrl ?? `${nodeUrl}/ext/bc/T/rpc`,
