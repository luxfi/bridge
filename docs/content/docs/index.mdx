...
```
┌─────────────────────────────────────────────────────────────┐
│  Tenant app (Lux, Hanzo, Zoo, …) — ~120 LOC Vite SPA        │
│  imports @luxfi/bridge + @<org>/brand                        │
└────────────────────────┬────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
│  @luxfi/bridge  — public SDK (pkg/bridge)                   │
│  Bridge UI INLINED under src/app/ (no lazy workspace deps)  │
│  Direct deps: @hanzo/gui (Tamagui), @luxfi/threshold,       │
│               wagmi, viem, @tanstack/react-query            │
└─────────────┬────────────────────────────┬──────────────────┘
              │                            │
┌─────────────▼──────────────┐   ┌─────────▼──────────────────┐
│ app/server                 │   │ @luxfi/threshold (T-Chain) │
│ Bridge API                 │   │ MPC threshold-sign SDK     │
│ Express + Prisma           │   │ (consumed by BOTH the      │
│ api.bridge.lux.network     │   │  bridge SDK + app/server)  │
└─────────────┬──────────────┘   └─────────┬──────────────────┘
              │                            │
┌─────────────▼──────────────┐   ┌─────────▼──────────────────┐
│ Infrastructure             │   │ m-chain — public MPC       │
│ KMS, Lux ID (Casdoor),     │   │ 2-of-3 threshold network   │
│ NATS, Consul, PostgreSQL   │   │ + optional private cluster │
└─────────────┬──────────────┘   │ + optional layered cosign  │
              │                  │   (Utila / Fireblocks)     │
              │                  └────────────────────────────┘
┌─────────────▼───────────────────────────────────────────────┐
│  Blockchain Networks                                         │
...
Native protocol set (`BridgeMPCConfig.protocol`):

| Class | Protocols |
|---|---|
| Classical ECDSA | `cggmp21` (default), `doerner` |
| Classical EdDSA | `frost` |
| Classical BLS | `bls` |
| **PQ-safe lattice** | `pulsar`, `corona`, `magnetar` |

The last three are post-quantum-safe and leaderless by construction.
...
- **Optional layered cosigners** (since SDK v1.0.3): Utila or Fireblocks
  as additional cosigners on top of native MPC; backend enforces 2-of-2
  (native + listed) before releasing settlement
