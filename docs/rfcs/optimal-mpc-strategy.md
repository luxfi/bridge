...
### 3. Quantum Option: Corona (Lattice-based)
```
Pros:
✅ Quantum resistant
✅ 2-round protocol
...
Pros:
✅ Quantum resistant
✅ 2.4 KB signatures (5.5x smaller than Corona)
✅ NIST standardized
...
- Consider Corona if threshold PQ is critical

### Option B: Quantum-First (If High Security Required)

```yaml
# Use Corona despite overhead
implementation_plan:
  treasury:
    scheme: corona
    threshold: 5-of-7
  ...
  users:
    scheme: cggmp21  # Not corona due to cost
    threshold: 10-of-15
    frequency: > 1000/day
```
...
3. **Research vs Production**:
   - Corona: Academic implementation
...
- If quantum requirement now → Consider Corona for treasury
...
1. **Upgrade to CGGMP21** (not Corona) for everyday operations
...
2. **Add Dilithium** for treasury operations
   - Quantum safe
   - 5.5x smaller than Corona
   - NIST approved

3. **Keep Corona** as emergency option
   - If true threshold PQ becomes critical
   - If regulations require it
   - If quantum timeline accelerates

**Corona is impressive research**, but its 206x signature size and 6000x network overhead make it impractical for a production bridge. CGGMP21 + selective Dilithium gives you the best of both worlds: efficiency for users and quantum safety for treasury.