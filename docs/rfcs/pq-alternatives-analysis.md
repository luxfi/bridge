...
You're right to ask - Corona's 140x signature size and 6000x network overhead is impractical. Let's examine more performant post-quantum alternatives.
...
Signature Size: 2,420 bytes (vs Corona's 13,400)
...
**Cons:**
- ❌ Large signatures (but smaller than Corona)
...
| Corona | 13,400 bytes | 140x | ✅ Yes | ❌ No | 🟡 Research |
...
if transfer.Value > TREASURY_THRESHOLD {
    ...
    // 2.4 KB signature vs 13.4 KB Corona
}
```
...
## Cost Analysis: Dilithium vs Corona vs BLS
...
1. **Dilithium**: 5.5x smaller than Corona, NIST standardized
2. **Falcon**: Only 7x larger than BLS, smallest PQ option
3. **Hybrid approaches**: Smooth transition path

**Recommended approach:**
1. Use **Dilithium** instead of Corona for treasury operations (better performance)
...
The key insight: You don't need threshold signatures for treasury operations if you have a secure multi-party approval process. Standard Dilithium with MPC approval gives you PQ security with 5.5x better performance than Corona.