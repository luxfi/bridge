...
4. **Quantum Preparedness**: Corona integration for post-quantum security
...
├─────────────────────────────────────────────────────────┤
│ Corona (Post-Quantum Key Root)                       │
└─────────────────────────────────────────────────────────┘
```

**Key Derivation**: All threshold protocols derive keys from Corona's PQ root using `HKDF(curveID || coronaShare)`, ensuring quantum-safe key anchoring while supporting classical signature schemes.
...
2. **Migration Strategy**: Gradual migration to Corona-based schemes
...
The modular architecture ensures each signature scheme can be optimized independently while maintaining a unified security model rooted in Corona's post-quantum foundation.
