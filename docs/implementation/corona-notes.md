# Corona: Practical Two-Round Lattice-Based Threshold Signatures

## Overview

Corona is a state-of-the-art lattice-based threshold signature scheme that provides post-quantum security. Unlike the current GG18 ECDSA implementation, Corona is based on the Learning with Errors (LWE) and Short Integer Solution (SIS) problems, making it resistant to quantum attacks.
...
| Feature | GG18 (Current) | Corona (Proposed) |
|---------|----------------|---------------------|
...
Node.js API → Rust Corona Binary → Lattice Signature
```
...
3. Implement core Corona protocol
...
3. Modify smart contracts to verify Corona signatures
4. Update bridge protocol for post-quantum signatures

#### Phase 3: Migration
1. Support both ECDSA and Corona in parallel
2. Gradual migration path for existing assets
3. Performance optimization

## Building on CGGMP Foundation

While Corona is fundamentally different from CGGMP (lattice vs elliptic curve), we can reuse several architectural patterns:
...
1. **Prototype Development**
   - Start with standalone Corona implementation
...
1. Boschini et al., "Corona: Practical Two-Round Threshold Signatures from Learning with Errors"
...
This provides a foundation for implementing Corona as a post-quantum alternative to the current ECDSA-based MPC system.