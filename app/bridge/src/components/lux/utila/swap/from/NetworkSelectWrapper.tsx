...
export default function NetworkSelectWrapper<T>({
  ...
  return (
    <>
      <div className="flex items-center relative">
        ...
        >
          <span className="flex grow text-left items-center text-xs md:text-base">
            ...
            {network ? (
              <span className="ml-3 block font-medium text-muted flex-auto items-center">{network.display_name}</span>
            ) : (
              <span className="block font-medium text-muted-2 flex-auto items-center">{placeholder}</span>
            )}
          </span>
      ...
      <NetworkSelect setShow={setShowModal} setNetwork={handleSelect} show={showModal} network={network} searchHint="" networks={networks} />
    </>
  )
}
