...
> = ({
  ...
  const {
    ...
    connectWallet,
    disconnectWallet,
    getAutofillProviderUsingName: getProvider,
  } = useWallet()

  const provider = React.useMemo(() => {
    return network && getProvider(network.internal_name)
  }, [network, getProvider])
        ...
        <div className="flex flex-col self-center grow w-full space-y-3">
            ...
            <div className="flex flex-wrap flex-col md:flex-row">
              <div className="relative flex grow rounded-lg shadow-sm mt-1.5 bg-level-1 border-[#404040] border focus-within:ring-0 focus-within:ring-foreground focus-within:border-foreground">
                ...
                <input
                  ...
                  tabIndex={0}
                  className={
                    (isPartnerWallet ? 'pl-11' : '') +
                    'px-2 disabled:cursor-not-allowed grow h-12 border-none leading-4  block font-semibold w-full bg-level-1 rounded-lg truncate hover:overflow-x-scroll focus:ring-0 focus:outline-none'
                  }
                />
                {inputValue && !disabled && (
              ...
              {wrongNetwork && !inputValue && (
                <div className="basis-full text-xs text-primary">
                  ...
                  KnownInternalNames.Networks.StarkNetMainnet ? (
                    <span>
                      Please switch to Starknet Mainnet with your wallet and
                      click Autofill again
                    </span>
                  ) : (
                    <span>
                      Please switch to Starknet Goerli with your wallet and
                      click Autofill again
                    </span>
                  )}
                </div>
          ...
          {validInputAddress && (
            <div
              ...
              className="text-left min-h-12 cursor-pointer space-x-2 border border-muted bg-level-1 shadow-xl flex text-sm rounded-md items-center w-full transform hover:bg-level-2 transition duration-200 px-2 py-2 hover:border-foreground hover:shadow-xl"
            >
