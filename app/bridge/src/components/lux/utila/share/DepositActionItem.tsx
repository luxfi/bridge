...
import shortenAddress, { capitalizeFirstCharacter } from "@/components/utils/ShortenAddress";
...
const DepositActionItem: React.FC<{
  ...
  const Address = ({name, value, isTx = false}: {name: string, value: string, isTx?: boolean}) => (
    <div className="flex items-center gap-2">
      <span className="text-[#22C55E]">{name}: </span>
      <a 
        href={!isTx ? network?.account_explorer_template?.replace("{0}", value) : network?.transaction_explorer_template?.replace("{0}", value)}
        target="_blank"
  ...
  return (
    <div>
      <div className="text-sm flex items-end gap-2">
        ...
        <div>
          { capitalizeFirstCharacter(data.status) } <span className="text-[#22C55E]">{data.amount}</span> {asset.asset} Transfer
        </div>
        <div className="hover:opacity-70 cursor-pointer" onClick={() => setOpen(!open)}>
