...
const getGroupName = (value: CryptoNetwork | Exchange, type: "cex" | "layer") => {
...
const NetworkFormField = forwardRef(function NetworkFormField(
  ...
  let placeholder = ""
  let searchHint = ""
  let filteredLayers: CryptoNetwork[]
  ...
  if (direction === "from") {
    ...
    filteredLayers = networks.filter(
      (l) =>
        ...
        <div className="flex justify-between items-center mt-2 pl-3 pr-4">
          {direction === "from" ? (
            ...
            <div className="font-semibold md:font-bold text-right leading-4">
              <p>
                <span>{parsedReceiveAmount}</span>&nbsp
...
function GenerateMenuItems(
  ...
  const mappedLayers = networks
    .map((l) => {
      ...
      const res: SelectMenuItem<CryptoNetwork> = {
        baseObject: l,
        id: l.internal_name,
        name: l.display_name,
        order: order || 100,
