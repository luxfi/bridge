...
import {
  generateSwapInitialValues,
  generateSwapInitialValuesFromSwap,
} from '@/lib/generateSwapInitialValues'
...
import { UserType, useAuthDataUpdate } from '@/context/auth-provider'
...
export default function Form() {
  ...
  const { data: partnerData } = useSWR<ApiResponse<Partner>>(
    query?.appName && `/apps?name=${query?.appName}`,
    client.fetcher
  )
  const partner =
    query?.appName &&
    partnerData?.data?.name?.toLowerCase() ===
      (query?.appName as string)?.toLowerCase()
      ? partnerData?.data
      : undefined
  ...
  const handleSubmit = useCallback(
    async (values: SwapFormValues) => {
      try {
        ...
        mutate(
          `/swaps?status=${SwapStatusInNumbers.Pending}&version=${BridgeApiClient.apiVersion}`
        )
      } catch (error: any) {
        const data: ApiError = error?.response?.data?.error
        if (data?.code === LSAPIKnownErrorCode.BLACKLISTED_ADDRESS) {
          toast.error(
            "You can't transfer to that address. Please double check."
          )
        } else if (data?.code === LSAPIKnownErrorCode.INVALID_ADDRESS_ERROR) {
          toast.error(`Enter a valid ${values.to?.display_name} address`)
        } else if (
          data?.code === LSAPIKnownErrorCode.UNACTIVATED_ADDRESS_ERROR &&
          values.to
        ) {
          setNetworkToConnect({
  ...
  const initialValues: SwapFormValues = swap
    ? generateSwapInitialValuesFromSwap(swap, settings)
    : generateSwapInitialValues(settings, query)
  const initiallyValidation = MainStepValidation({
    minAllowedAmount,
    maxAllowedAmount,
  })(initialValues)
  const initiallyIsValid =
    Object.values(initiallyValidation)?.filter((v) => v).length > 0
  ...
  const handleClosesSwapModal = () => {
    ...
    window.history.replaceState(
      { ...window.history.state, as: homeURL, url: homeURL },
      '',
      homeURL
    )
  }
