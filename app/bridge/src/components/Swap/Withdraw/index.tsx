...
const Withdraw: FC = () => {
  ...
  const source = networks.find((n) => n.internal_name === source_internal_name);

  const sourceIsStarknet =
    swap?.source_network?.toUpperCase() ===
      KnownInternalNames.Networks.StarkNetMainnet?.toUpperCase() ||
    swap?.source_network ===
      KnownInternalNames.Networks.StarkNetGoerli?.toUpperCase();
  const sourceIsImmutableX =
    swap?.source_network?.toUpperCase() ===
      KnownInternalNames.Networks.ImmutableXMainnet?.toUpperCase() ||
    swap?.source_network ===
      KnownInternalNames.Networks.ImmutableXGoerli?.toUpperCase();
  const sourceIsZkSync =
    swap?.source_network?.toUpperCase() ===
    KnownInternalNames.Networks.ZksyncMainnet?.toUpperCase();
  const sourceIsArbitrumOne =
    swap?.source_network?.toUpperCase() ===
      KnownInternalNames.Networks.ArbitrumMainnet?.toUpperCase() ||
    swap?.source_network ===
      KnownInternalNames.Networks.ArbitrumGoerli?.toUpperCase();
  ...
  const isImtblMarketplace =
    signature && appName === "imxMarketplace" && sourceIsImmutableX;
  const sourceIsSynquote =
    appName === "ea7df14a1597407f9f755f05e25bab42" && sourceIsArbitrumOne;
        ...
        <div className="w-full flex flex-col justify-between  ">
          <div className="grid grid-cols-1">
            ...
            <span>
              {showTabsHeader && (
                <>
                  <div className="mb-4 ml-1 text-base">
...
const WalletTransferContent: FC = () => {
  ...
  const canOpenAccount =
    sourceNetworkType === NetworkType.EVM && !swap?.source_exchange;
  ...
  if (
    !accountAddress ||
    (swap?.source_exchange && !swap.exchange_account_connected)
  ) {
    return (
      <div className="flex justify-center">
        <WalletIcon className="w-12 text-muted-3" />
      </div>
    );
  }
  ...
  return (
    <div className="grid content-end">
      ...
      <div
        ...
        className={
          (canOpenAccount ? "cursor-pointer" : "cursor-auto") +
      ...
      >
        <div className="flex bg-level-3 flex-row items-left rounded-md p-1">
          {!swap?.source_exchange && wallet?.connector && (
            <wallet.icon className="w-6 h-6 rounded-full" />
          )}
          {source_exchange && (
            <Image
        ...
        <div className="flex flex-col grow">
          <div className="block text-md font-medium ">
            ...
            {swap?.source_exchange && (
              <span>{shortenEmail(swap?.exchange_account_name)}</span>
            )}
          </div>
        </div>
        <div
          ...
          className={
            "cursor-pointer flex flex-row items-left p-2 rounded-md transform " +
