...
import { type Dispatch, type PropsWithChildren, type SetStateAction, useCallback, useEffect, useRef } from 'react'
    ...
    return (
        <div ref={topmostRef}>
            ...
            <motion.div
                key="mobile-modal"
                ref={mobileModalRef}
                className={`${wrapperHeightClass} max-h-full overflow-y-auto group ${position} inset-x-0 bottom-0 z-40 w-full ${height != 'full' ? 'rounded-t-2xl border-t border-[#404040]' : ''}  bg-background ${className} shadow-lg`}
                initial={{ y: "20%" }}
                animate={controls}
                exit={{ y: "100%" }}
                transition={transitionProps}
            ...
            >
                <div className={`py-3 flex flex-col h-full z-40 ${height != 'full' ? 'bg-background border-t border-[#404040] rounded-t-2xl ' : ''} pb-6`}>
