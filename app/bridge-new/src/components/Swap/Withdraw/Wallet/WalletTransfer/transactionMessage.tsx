...
const TransactionMessage: React.FC<TransactionMessageProps> = ({
    ...
    if (wait?.isLoading || applyingTransaction) {
        ...
        const unexpectedError = prepare?.error
            || (transaction?.error as any)?.['data']?.message || transaction?.error
            || wait?.error
...
const PreparingTransactionMessage: React.FC = () => {
...
const ConfirmTransactionMessage: React.FC = () => {
    return <WalletMessage
        status="pending"
...
const TransactionInProgressMessage: React.FC = () => {
    return <WalletMessage
        status="pending"
...
const InsufficientFundsMessage: React.FC = () => {
    return <WalletMessage
        status="error"
...
const TransactionRejectedMessage: React.FC = () => {
    return <WalletMessage
        status="error"
...
const UexpectedErrorMessage: React.FC<{ message: string }> = ({ message }) => {
    return <WalletMessage
        status="error"
