        ...
        <li className={classNames(isLastStep ? '' : 'pb-10', 'relative')} key={step?.name}>
            <div className="flex items-center justify-between w-full">
                ...
                <div className={`group relative flex ${step?.description ? "items-start" : "items-center"}`}>
                    <span className="flex h-9 items-center text-muted text-muted-primary-text" aria-hidden="true">
                        {renderStepIcon(step)}
                    </span>
                    <span className="ml-3 flex min-w-0 flex-col">
                        <span className={`text-sm font-medium ${step.status === "current" ? "text-primary" : step.status === "upcoming" ? "text-foreground text-foreground-new/70" : "text-muted text-muted-primary-text"}`}>{step.name}</span>
                        {step?.description &&
                            <span className="text-sm text-foreground text-foreground-new">{step?.description}</span>}
                    </span>
                </div>
            </div>
        </li>
