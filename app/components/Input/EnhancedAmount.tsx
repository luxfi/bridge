...
const EnhancedAmountField = forwardRef(function EnhancedAmountField(_, ref: any) {
  ...
  const provider = useMemo(() => (
    values.from && getProvider(values.from)
  ...
  return (
    <NumericInput
      ...
      className="rounded-r-none text-muted text-muted-primary-text"
    >
      {from && to && currency && (

        <div className="text-xs flex items-center ">
          <SecondaryButton onClick={handleSetMinAmount} size="xs" className='mr-1'>
            MIN
          </SecondaryButton>
...
const AmountLabel = ({
  ...
  <div className="w-full flex flex-row items-center p-2">
    ...
    {
      detailsAvailable &&
      <div className="text-xs hidden md:flex items-center pl-1.5">
    ...
    {walletBalance != undefined && !isNaN(walletBalance) && (
      <div className="border-primary-text text-xs text-muted text-muted-primary-text flex items-center space-x-1">
        <span>Balance:</span>
        {isBalanceLoading ?
