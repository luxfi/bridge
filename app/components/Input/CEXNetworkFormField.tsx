...
const CEXNetworkFormField = forwardRef(function CEXNetworkFormField(
  { direction }: Props,
  ref: any
) {
  ...
  const value = menuItems?.find(
    (item) =>
      ...
      item.baseObject.network ===
        (direction === "from" ? from : to)?.internal_name
  );
  ...
  return (
    <div className="flex justify-between items-center w-fit gap-1">
      ...
      <div className="w-fit" ref={ref}>
        <Select
          value={value?.id}
          onValueChange={(v) =>
            handleSelect(menuItems.find((m) => m.id === v)!)
          }
        >
