...
export default class BridgeApiClient {
    ...
    constructor(private readonly _router?: NextRouter, private readonly _redirect?: string) {
        this._authInterceptor = InitializeInstance(BridgeAuthApiClient.identityBaseEndpoint);
    }
    ...
    async CreateSwapAsync(params: CreateSwapParams): Promise<ApiResponse<CreateSwapData>> {
        ...
        return await this.AuthenticatedRequest<ApiResponse<CreateSwapData>>("POST", `/swaps?version=${BridgeApiClient.apiVersion}`, params, { 'X-LS-CORRELATION-ID': correlationId });
    }

    async GetSwapsAsync(page: number, status?: SwapStatusInNumbers): Promise<ApiResponse<SwapItem[]>> {
        return await this.AuthenticatedRequest<ApiResponse<SwapItem[]>>("GET", `/swaps?page=${page}${status ? `&status=${status}` : ''}&version=${BridgeApiClient.apiVersion}`);
    }

    async GetPendingSwapsAsync(): Promise<ApiResponse<SwapItem[]>> {
        return await this.AuthenticatedRequest<ApiResponse<SwapItem[]>>("GET", `/swaps?status=0&version=${BridgeApiClient.apiVersion}`);
    }
    ...
    async GetSwapDetailsAsync(id: string): Promise<ApiResponse<SwapItem>> {
        return await this.AuthenticatedRequest<ApiResponse<SwapItem>>("GET", `/swaps/${id}?version=${BridgeApiClient.apiVersion}`);
    }
    ...
    async GetFee(params: GetFeeParams): Promise<ApiResponse<any>> {
        return await this.AuthenticatedRequest<ApiResponse<any>>("POST", `/swaps/quote?version=${BridgeApiClient.apiVersion}`, params);
    }

    private async AuthenticatedRequest<T extends EmptyApiResponse>(method: Method, endpoint: string, data?: any, header?: {}): Promise<T> {
        let uri = BridgeApiClient.apiBaseEndpoint + "/api" + endpoint;
