...
export default function resolveChain(
  ...
  const nativeCurrency = network.currencies.find((c: Asset) => c.asset === network.native_currency)
  ...
  if (!nativeCurrency) {
    SendErrorMessage(
      'UI Settings error',
      `env: ${process.env.NEXT_PUBLIC_VERCEL_ENV} %0A url: ${process.env.NEXT_PUBLIC_VERCEL_URL} %0A message: could not find native currency for ${network.internal_name} %0A`
    )
    return
  }
  ...
  const wagmiChain = {
    ...
    rpcUrls: {
      default: {
        http: network.nodes.map((n) => n as string),
      },
      public: {
        http: network.nodes.map((n) => n as string),
      },
