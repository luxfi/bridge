...
func TestSigningDriver_TON_HappyPath_AssemblesBOC(t *testing.T) {
	...
	if err := store.PutEntry(context.Background(), FamilyTON, 0, entry); err != nil {
		t.Fatal(err)
	}
	pool := NewReleasePoolForFamily(store, FamilyTON, "TON_TESTNET", nil)
...
func TestSigningDriver_TON_MissingEdDSAPubKey_Rolls_Back(t *testing.T) {
	...
	_ = store.PutEntry(context.Background(), FamilyTON, 0, entry)
	pool := NewReleasePoolForFamily(store, FamilyTON, "TON_TESTNET", nil)
