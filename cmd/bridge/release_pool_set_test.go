...
func TestReleasePoolSet_RoutesByFamily(t *testing.T) {
	...
	evmStore, _ := NewInMemoryStoreFamily(store, FamilyEVM)
	solStore, _ := NewInMemoryStoreFamily(store, FamilySOL)

	evmPool := NewReleasePool(evmStore, "LUX_TESTNET", nil)
	solPool := NewReleasePool(solStore, "SOLANA_DEVNET", nil)
...
func TestInMemoryStoreFamily_Isolation(t *testing.T) {
	...
	evm, _ := NewInMemoryStoreFamily(base, FamilyEVM)
	sol, _ := NewInMemoryStoreFamily(base, FamilySOL)

	_ = evm.PutEntry(context.Background(), 0, ReleasePoolEntry{Index: 0, WalletID: "evm-0"})
	_ = sol.PutEntry(context.Background(), 0, ReleasePoolEntry{Index: 0, WalletID: "sol-0"})

	evmEntries, _ := evm.LoadEntries(context.Background())
	solEntries, _ := sol.LoadEntries(context.Background())
...
func TestInMemoryStoreFamily_AscendingOrder(t *testing.T) {
	...
	for _, idx := range []int{2, 0, 1} {
		_ = sol.PutEntry(context.Background(), idx, ReleasePoolEntry{
			Index: idx, WalletID: fmt.Sprintf("w-%d", idx),
		})
	}
	got, _ := sol.LoadEntries(context.Background())
	if len(got) != 3 {
...
func TestFamilyCode_Constants(t *testing.T) {
	...
	if FamilyEVM != "eth" {
		t.Errorf("FamilyEVM = %q, want \"eth\"", FamilyEVM)
	}
	if FamilySOL != "sol" {
