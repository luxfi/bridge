...
export default function SearchData({ searchParam }: { searchParam: string }) {
  ...
  const { data, error, isLoading } = useSWR<ApiResponse<Swap[]>>(`${AppSettings.BridgeApiUri}/api/explorer/${searchParam}?version=${version}`, fetcher, { dedupingInterval: 60000 });
  ...
  const DestinationNativeToken = destinationLayer?.assets?.find(a => a.is_native);
    ...
    return (
      <tr key={index} onClick={(e) => router.push(`/${inputTransaction?.transaction_id}`)} className="hover:bg-level-2 hover:cursor-pointer">
        <td className="whitespace-nowrap border-l border-r border-b border-muted-4 py-2 px-3 text-sm font-medium">
            ...
            <div className=""><StatusIcon swap={swap.status} className='mr-1' />{new Date(swap.created_date).toLocaleString()}</div>
        </td>
        ...
        <td className={"whitespace-nowrap border-b border-muted-4 text-sm mr-4 " +
  ...
  return (Number(data?.data?.length) > 1 ? (
    <div className="w-full px-6">
      ...
      <div className="flow-root w-full">
        <div className="p-4 mb-4 flex gap-1 rounded-lg border border-muted-4 border-1" style={{width: 'fit-content'}}>
            <span className="font-bold mr-1 ">Address:</span>
            <span className="break-all">
                {swap?.destination_address}
                <CopyButton toCopy={swap?.destination_address || ''} iconHeight={16} iconClassName="order-2" iconWidth={16} className="inline-flex items-center ml-3 align-middle" />
            </span>
        </div>
        <div className={resultsCouldScroll ? 
          'overflow-y-auto h-full max-h-[55vh] 2xl:max-h-[65vh] dataTable' : 'overflow-hidden'
        }>
              ...
              <thead className="sticky top-0 z-10 bg-background">
                <tr>
                  ...
                  <th scope="col" className={
                    ...
                    (Number(filteredData?.length) > 5 ? '' : 'border-r rounded-tr-lg ')
                  } />
                </tr>
              </thead>
              <tbody >
