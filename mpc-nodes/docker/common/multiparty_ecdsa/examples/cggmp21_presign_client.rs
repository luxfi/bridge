...
use multi_party_ecdsa::protocols::multi_party_ecdsa::gg_2021::party_i::PresignParty;
use multi_party_ecdsa::protocols::multi_party_ecdsa::gg_2021::state_machine::traits::{KeyShare, PresignData};
use std::{env, fs};
use reqwest::Client;
use serde_json::json;
...
fn main() {
    ...
    if env::args().nth(4).is_none() {
        panic!("Usage: {} <party_index> <threshold> <n_parties> <session_id>", env::args().nth(0).unwrap());
    }
    ...
    let session_id = env::args().nth(4).unwrap();
    ...
    // Initialize presign party
    let mut presign = PresignParty::new(party_index, threshold, session_id.clone(), key_share);
    ...
    println!("Party {} (index {}) joined presigning with UUID: {}", party_num_int, party_index, uuid);
    
    // Execute presigning protocol
    
    // Round 1: Generate and broadcast first message
    let round1_msg = presign.round1().expect("Failed to generate round 1 message");
    broadcast(
        &client,
        party_num_int,
        "round1",
    ...
    let round2_msg = presign.round2(round1_msgs_parsed).expect("Failed to generate round 2 message");
    broadcast(
        &client,
        party_num_int,
        "round2",
    ...
    let round3_msg = presign.round3(round2_msgs_parsed).expect("Failed to generate round 3 message");
    broadcast(
        &client,
        party_num_int,
        "round3",
    ...
    // Finalize: Process round 3 messages and generate presign data
    let presign_data = presign.finalize(round3_msgs_parsed).expect("Failed to finalize presigning");
    
    // Output the presign data to stdout
    let presign_data_json = serde_json::to_string(&presign_data).unwrap();
    println!("{}", presign_data_json);
    
    // Also save to file for convenience
    let filename = format!("presign_{}_{}.json", session_id, party_index);
    fs::write(&filename, &presign_data_json).expect("Unable to save presign data");
    
    println!("Presigning completed successfully!");
    println!("Presign data saved to {}", filename);
}

// Sign up for the protocol
fn signup(client: &Client) -> Result<PartySignup, ()> {
    let key = "signup-presign".to_string();
    
    let res_body = postb(client, "signuppresign", key).unwrap();
