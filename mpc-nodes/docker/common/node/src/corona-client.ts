...
// Types for Corona signatures
export interface CoronaSignature {
...
export interface CoronaSignRequest {
    session_id: string;
...
export interface CoronaSignResponse {
    success: boolean;
    round: number;
...
export interface CoronaClientConfig {
    partyId: number;
    serviceUrl: string;
    timeout?: number;
}
...
/**
 * Client for interacting with the Corona signing service
 */
export class CoronaClient {
    ...
    constructor(config: CoronaClientConfig) {
        this.partyId = config.partyId;
        this.logger = new Logger('CoronaClient');
    ...
    /**
     * Execute Round 1 of the Corona signing protocol (offline phase)
     */
    async signRound1(sessionId: string, message: string): Promise<string> {
        try {
            const request: CoronaSignRequest = {
                session_id: sessionId,
                message: message, // Should be hex encoded
                round: 1,
            };

            const response = await this.client.post<CoronaSignResponse>('/sign', request);
    ...
    /**
     * Execute Round 2 of the Corona signing protocol (online phase)
     */
    async signRound2(sessionId: string, message: string): Promise<CoronaSignature> {
        try {
...
/**
 * Factory function to create Corona client from environment
 */
export function createCoronaClient(): CoronaClient {
    const partyId = parseInt(process.env.PARTY_ID || '0', 10);
    const serviceUrl = process.env.CORONA_SERVICE_URL || 'http://localhost:8080';
    
    return new CoronaClient({
        partyId,
        serviceUrl,
    });
}
...
/**
 * Helper to convert Corona signature to hex string for storage
 */
export function coronaSignatureToHex(sig: CoronaSignature): string {
    return JSON.stringify(sig);
}

/**
 * Helper to parse Corona signature from hex string
 */
export function hexToCoronaSignature(hex: string): CoronaSignature {
