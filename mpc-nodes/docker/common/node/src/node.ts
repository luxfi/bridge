...
app.post("/api/v1/generate_mpc_sig", signDataValidator, async (req: Request, res: Response) => {
  ...
  // XRPL path: handle XRP mainnet, testnet, and devnet
  if (
    fromNetwork.internal_name === "XRPL_MAINNET" ||
    fromNetwork.internal_name === "XRPL_TESTNET" ||
    fromNetwork.internal_name === "XRPL_DEVNET"
  ) {
...
app.post("/api/v1/complete", async (req: Request, res: Response) => {
  const { hashedTxId } = req.body
  console.log({hashedTxId})
  try {
    const tx = await prisma.teleporter.findUnique({
      where: { hashedTxId: hashedTxId }
    })
    if (tx) {
      await prisma.teleporter.update({
        where: { hashedTxId: hashedTxId },
        data: { completed: true }
      })
